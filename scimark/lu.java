    ...
    */
    ...
    protected static final void insert_copy(double B[][], double A[][]) {
        ...
        for (int i = 0; i < M; i++) {
            double Bi[] = B[i];
            double Ai[] = A[i];
            for (int j = 0; j < remainder; j++) Bi[j] = Ai[j];
    ...
    */
    public static int factor(double A[][], int pivot[]) {
        ...
        for (int j = 0; j < minMN; j++) {
            ...
            if (A[jp][j] == 0) return 1; // factorization failed because of zero pivot
